...
void thread_init (void) {
	...
	list_init (&sleep_list);  // newly added
...
void thread_unblock (struct thread *t) {
	...
	list_insert_ordered(&ready_list, & t->elem, cmp_priority, NULL);  // replaced
...
void thread_yield (void) {
	...
	if (curr != idle_thread) {
		...
		list_insert_ordered(&ready_list, &curr->elem, cmp_priority, NULL);  // replaced
	}
...
void test_max_priority(void) {
	...
	if (!list_empty(&ready_list)) {
		struct thread *top_priority = list_begin(&ready_list);
		// 현재 쓰레드의 우선수위가 더 작다면 thread_yield()
		if (cmp_priority(top_priority, &thread_current()->elem, NULL))
			thread_yield();
	}
}
